    ...
    <div className="max-w-3xl mx-auto">
      <div className="text-center mb-12">
        ...
        <h2 className="text-3xl md:text-5xl text-[#1C1E21] tracking-tight leading-[1.05] mb-4">
          Quel est ton <span className="italic text-[#1877F2]">métier</span> ?
        </h2>
          ...
          return (
            <button
              ...
              className={`group text-left p-5 rounded-2xl border-2 transition-all duration-200 ${
                isSelected
                  ? "border-[#1877F2] bg-[#F7F8FA] shadow-md"
                  : "border-[#1C1E21]/10 hover:border-[#1C1E21]/30 bg-[#F7F8FA]/50"
              }`}
            >
              <span className="text-3xl block mb-3">{b.emoji}</span>
              <h3 className={`text-lg mb-1 leading-tight ${isSelected ? "text-[#1877F2]" : "text-[#1C1E21]"}`}>
                {b.label}
              </h3>
              <p className="text-xs text-[#1C1E21]/60 leading-relaxed">{b.desc}</p>
            </button>
          );
        })}
      </div>
